...
object ListIntUtils {
   ...
   * Busca el Maximo
   */
  def max(lista: List[Int]) : Int = if (lista.tail.isEmpty || lista.head > max(lista.tail)) lista.head else max(lista.tail)
  
  /*
   * Busca el minimo
   */
  def min(lista: List[Int]) : Int = if (lista.tail.isEmpty || lista.head < min(lista.tail)) lista.head else min(lista.tail)
