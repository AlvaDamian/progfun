...
object ArbolBinario {
  ...
  def insert(nro: Int, tree: Tree): Tree = tree match {
    ...
    case Node(nroN, left, rigth) => if (nro < nroN) Node(nroN, insert(nro,left), rigth)
                                    else Node(nroN, left, insert(nro,rigth))
  }
  ...
  def elem(nro: Int, tree: Tree): Boolean = tree match {
    ...
    case Node(n, left, right) => n == nro ||
                                (n > nro && elem(nro, left)) ||
                                (n < nro && elem(nro, right))
  }
   ...
   */
  def double(tree: Tree): Tree = apply(tree, (a) => a * 2)
